...
async fn main() -> Result<()> {
    ...
    // Find description
    let current_desc = log.graph.iter()
    ...
    let log_new = adapter.get_operation_log().await?;
    let new_wc_id = &log_new.working_copy_id;
    println!("New Working Copy ID: {}", new_wc_id);
    
    let new_entry_desc = log_new.graph.iter()
    ...
    if new_entry_desc == new_desc {
        ...
        println!("FAILURE: Description mismatch! Expected '{}', got '{}'", new_desc, new_entry_desc);
    }
